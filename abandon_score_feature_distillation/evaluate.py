# evaluate.py
# (models.py의 3-value 반환값을 처리하도록 수정 + Confusion Matrix 추가)
...
# ❗️[수정] sklearn.metrics 추가
from sklearn.metrics import confusion_matrix
from torch.utils.data import DataLoader, TensorDataset, random_split
from train_utils import metrics_dict, resolve_device


# ============================================================
# 1. Baseline (A only) 모델 정의 (❗️ [핵심 수정])
# ============================================================
class BaselineNet(nn.Module):
    """
    [수정] models.py의 MLP가 (output, features) 2개를 반환하므로,
    이를 처리하도록 BaselineNet의 forward 함수를 수정합니다.
    """
    ...
    def forward(self, x):
        # ❗️[수정] MLP가 2개 값을 반환 (z, features)
        z, _ = self.encoder(x) 
        # ❗️[수정] MLP가 2개 값을 반환 (y, features)
        y, _ = self.classifier(z) 
        
        # (이전과 동일하게 y, z 2개만 반환)
        return y, z

# ============================================================
# 2. [NEW] Confusion Matrix 시각화 함수
# ============================================================
def plot_confusion_matrices(Y_val, Yp_teacher, Yp_student, Yp_base, eval_dir, labels=[0, 1]):
    Y_val_np = Y_val.cpu().numpy().flatten()
    
    models_preds = {
        "Teacher (A+B)": Yp_teacher.cpu().numpy().flatten(),
        "Student (Distilled)": Yp_student.cpu().numpy().flatten(),
        "Baseline (A only)": Yp_base.cpu().numpy().flatten()
    }
    ...
    for ax, (model_name, y_pred) in zip(axes, models_preds.items()):
        cm = confusion_matrix(Y_val_np, y_pred, labels=labels)
    ...
    save_path = os.path.join(eval_dir, "eval_confusion_matrix.png")
...
def evaluate_all_models(cfg, A_tr, B_tr, Y_tr, A_val, B_val, Y_val, device):
    ...
    dim_A, dim_B = A_tr.shape[1], B_tr.shape[1]
    ...
    # (결과 저장을 위해 예측값을 담아둘 변수)
    Yp_teacher_classes, Yp_student_classes, Yp_base_classes = None, None, None
    ...
    with torch.no_grad():
        ...
        # ❗️ [수정] TeacherNet이 (logits, z_a, features) 3개를 반환
    ...
    with torch.no_grad():
        ...
        Yp_student_logits, _, _ = student(A_val) 
    ...
    for ep in range(cfg['common_train']['epochs']):
        baseline.train()
        for a_batch, y_batch in baseline_train_loader:
            opt.zero_grad()
            # ❗️ (수정된 BaselineNet 호출, y/z 2개 반환)
